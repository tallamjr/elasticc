...
from astronet.utils import astronet_logger, find_optimal_batch_size, load_dataset
...
class Objective(object):
    ...
    def __call__(self, trial):
        ...
        sgkf = StratifiedGroupKFold(n_splits=k_folds, shuffle=False, random_state=None)

        groups = np.load(f"{DPATH}/groups_train_idx.npy", mmap_mode="r")

        kth_fold = 1
        for i, (train_index, val_index) in enumerate(
            sgkf.split(X_train, y_train_split, groups)
        ):
            ...
            np.random.seed(RANDOM_SEED)
            mask = np.random.choice([False, True], len(X_train_cv), p=prop)
            X_train_cv = X_train_cv[mask]
            y_train_cv = y_train_cv[mask]

            np.random.seed(RANDOM_SEED)
            mask = np.random.choice([False, True], len(X_val_cv), p=prop)
            X_val_cv = X_val_cv[mask]
            y_val_cv = y_val_cv[mask]
            ...
            if self.redshift is not None:
                ...
                np.random.seed(RANDOM_SEED)
                mask = np.random.choice([False, True], len(Z_train_cv), p=prop)
                Z_train_cv = Z_train_cv[mask]

                np.random.seed(RANDOM_SEED)
                mask = np.random.choice([False, True], len(Z_val_cv), p=prop)
                Z_val_cv = Z_val_cv[mask]
